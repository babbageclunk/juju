...
func (s *StoreOperationSuite) TestClaimLease(c *gc.C) {
	...
	err := fix.Store.ClaimLease(key("name"), lease.Request{"holder", leaseDuration}, nil)
...
func (s *StoreOperationSuite) TestClaimMultipleLeases(c *gc.C) {
	...
	err := fix.Store.ClaimLease(key("short"), lease.Request{"holder", time.Second}, nil)
	c.Assert(err, jc.ErrorIsNil)
	err = fix.Store.ClaimLease(key("medium"), lease.Request{"grasper", time.Minute}, nil)
	c.Assert(err, jc.ErrorIsNil)
	err = fix.Store.ClaimLease(key("long"), lease.Request{"clutcher", time.Hour}, nil)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *StoreOperationSuite) TestCannotClaimLeaseTwice(c *gc.C) {
	...
	err = fix.Store.ClaimLease(key("name"), lease.Request{"other-holder", leaseDuration}, nil)
...
func (s *StoreOperationSuite) TestExtendLease(c *gc.C) {
	fix := s.EasyFixture(c)
	err := fix.Store.ClaimLease(key("name"), lease.Request{"holder", time.Second}, nil)
	c.Assert(err, jc.ErrorIsNil)

	leaseDuration := time.Minute
	err = fix.Store.ExtendLease(key("name"), lease.Request{"holder", leaseDuration}, nil)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *StoreOperationSuite) TestExtendLeaseCannotChangeHolder(c *gc.C) {
	...
	leaseDuration := time.Minute
	err = fix.Store.ExtendLease(key("name"), lease.Request{"other-holder", leaseDuration}, nil)
...
func (s *StoreOperationSuite) TestExtendLeaseCannotShortenLease(c *gc.C) {
	...
	err = fix.Store.ExtendLease(key("name"), lease.Request{"holder", time.Second}, nil)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *StoreOperationSuite) TestCannotExpireLeaseBeforeExpiry(c *gc.C) {
	...
	err = fix.Store.ExpireLease(key("name"), nil)
	c.Assert(err, gc.Equals, lease.ErrInvalid)
}
