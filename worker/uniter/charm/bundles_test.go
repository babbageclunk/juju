...
func (s *BundlesDirSuite) TestGet(c *gc.C) {
	...
	c.Assert(err, gc.ErrorMatches, regexp.QuoteMeta(`failed to download charm "cs:dummy/quantal/1" from API server: `)+`expected sha256 "...", got ".*"`)
	...
	c.Assert(err, gc.ErrorMatches, regexp.QuoteMeta(`failed to download charm "cs:spam/quantal/1" from API server: `)+`.* not found`)
	...
	c.Assert(err, gc.ErrorMatches, regexp.QuoteMeta(`failed to download charm "cs:dummy/quantal/1" from API server: aborted`))
}
